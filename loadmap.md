...
- [x] GitHubリポジトリを作成する（例：`weather-app`）
- [x] コードをプッシュする
...
Cloudflare Workers でプロキシを作成し、API キーを取得するよう変更
...
異なるオリジン（github.io → workers.dev）へのリクエストになるため、Cloudflare Workers で CORS の設定が必要
